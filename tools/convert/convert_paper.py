...
Usage:
    ...
    python3 convert_paper.py paper.pdf --board Edexcel --subboard AEA --year 2010 --paper P1
...
def build_request_body(system_prompt: str, pdf_b64: str) -> dict:
    return {
        "systemInstruction": {"parts": [{"text": system_prompt}]},
        "contents": [
            {
                "parts": [
                    {"text": "Convert the attached exam paper PDF to markdown now."},
...
def call_gemini(api_key: str, model: str, system_prompt: str, pdf_b64: str, max_retries: int = 4):
    url = API_URL.format(model=model) + "?key=" + api_key
    body = build_request_body(system_prompt, pdf_b64)
...
def main():
    ...
    print(f"Reading {args.pdf} ({size/1e6:.1f}MB) ...")
    with open(args.pdf, "rb") as f:
        pdf_b64 = base64.b64encode(f.read()).decode()

    print(f"Calling {args.model} ...")
    text, usage, result = call_gemini(api_key, args.model, system_prompt, pdf_b64)
    print(
